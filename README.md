...
#### 1. Backend (backend_main.py)
**Основной сервер на FastAPI**, развёрнутый на Railway.

**Ключевые возможности:**
- **Очередь задач через Redis** - FIFO очередь для каждого агента (`agent:{id}:tasks`)
- **PostgreSQL** - хранение проверок, агентов и результатов
- **WebSocket Pub/Sub** - real-time обновления через Redis каналы
- **Heartbeat система** - автоматическое определение offline агентов (>60 сек без heartbeat)
- **Регистрация агентов** - токен-based аутентификация
...
- ✅ CORS настройка для конкретных доменов
...
Backend запустится автоматически через `Procfile`: `uvicorn backend_main:app`
...
├── main_server.py               # 📝 Ранняя версия backend (для справки)
├── requirements.txt             # 📦 Python зависимости
├── Procfile                     # 🚂 Railway запуск команда
...
**Очередь задач:**
- **Redis FIFO** - каждый агент имеет свою очередь (`agent:{id}:tasks`)
- **Polling** - агенты опрашивают очередь каждые 5 секунд
- **Код:** `backend_main.py:426-439` (создание), `backend_main.py:598-618` (polling)

**Retry механизм:**
- **Exponential backoff** при ошибках polling (код: `agent_production.py:419-426`)
- **Повторные попытки** отправки результатов при сбое
- **Таймауты** на всех сетевых операциях

**PostgreSQL с ACID:**
- Транзакции для всех записей
- **Foreign keys** для целостности данных
- **Триггеры** для автообновления статусов (код: `database_migration.sql:82-116`)
...
```bash
POST /api/agents/register
X-Registration-Token: <мастер-токен>
{
  "name": "Agent-Moscow",
  "location": "Russia, Moscow"
}
# Возвращает: agent_id + api_token
```
**Код:** `backend_main.py:280-310`

**Heartbeat система:**
- Агент отправляет heartbeat каждые **30 секунд**
- Backend автоматически помечает offline если >60 сек без сигнала
- **Код:** `agent_production.py:436-459`, `backend_main.py:312-348`
